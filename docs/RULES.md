...
| **CBOM9999** | Diagnostics | Analyzer-only: a detector threw and did not run (results unknown, not clean) | — | n/a | n/a | — |
...
- **Generated code is analyzed**, in the editor and in the CLI alike: `*.g.cs`, `*.designer.cs`,
  `*.generated.cs` and files carrying an `// <auto-generated/>` header. Crypto genuinely lives there (gRPC/
  WCF/OpenAPI clients, EF migrations, source-generated config), and excluding it would make the editor
  disagree with the CBOM. Silence a rule per-glob where you cannot edit the source:
  ```ini
  [*.g.cs]
  dotnet_diagnostic.CBOM0002.severity = none
  ```
- **CBOM9999** is the analyzer's fail-closed signal: if a detector throws, that rule reports once per
  compilation instead of vanishing. The CLI records the same failure as a scan diagnostic. A rule that did
  not run is *unknown*, not clean.
